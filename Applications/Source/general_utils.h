...
/* rahel includes */
#include "rahels_types.h"
#define MAX_STRING_LENGTH 128
static char data[MAX_STRING_LENGTH];
static void serial_read_until(char terminator_char){
    memset(data, MAX_STRING_LENGTH, '\0');
    unsigned int i = 0;
    char c;
    c = getchar();
    while (c  != terminator_char )
    {
...
static void pin_init(unsigned int pin_number, unsigned int  dir){
...
static void pins_init_all(unsigned int* pins, pin_direction* directions, unsigned int num_pins)
{
    for (unsigned int i = 0; i < num_pins; i++)
    {
        pin_init(pins[i], (unsigned int)directions[i]);
    }
}

/*-----------------------------------------------------------*/
static void serial_init(unsigned int baud){
    stdio_usb_init();
    stdio_uart_init_full(	
            uart_default, //uart_inst_t * 	uart,
            baud, //baud rate 
